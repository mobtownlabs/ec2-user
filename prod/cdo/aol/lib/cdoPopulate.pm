...
sub populatePVAvg
{
	...
	{
		## Blow Out Old Table
		my $sth=$dbi->prepare("TRUNCATE PriceVolumeAvg");
		$sth->execute();
		$sth->finish();
		
		my $sql = qq|INSERT INTO PriceVolumeAvg
               		     SELECT siteid,segmentid,size,avg(bid),allocation,avg(volume),avg(capped_volume) 
                             FROM PriceVolumeTest 
                             GROUP BY siteid,segmentid,size,allocation|;

     	my $sth = $dbi->prepare($sql);
     	$sth->execute();
     	
     	$logger->write($log, "\n     -> ".$sth->rows()." rows inserted into PriceVolumeAvg.");
     	$sth->finish();

		## Blow Out Old Table
		my $sth=$dbi->prepare("TRUNCATE EVolumeAvg");
		$sth->execute();
		$sth->finish();
		
		my $sql = qq|INSERT INTO EVolumeAvg
               			SELECT DISTINCT siteid, segmentid, size, volume, capped_volume
               			FROM PriceVolumeAvg|;

     	my $sth = $dbi->prepare($sql);
     	$sth->execute();
     	
     	$logger->write($log, "\n     -> ".$sth->rows()." rows inserted into EVolumeAvg.");
     	$sth->finish();
	...
	{
		$logger->write($log, "\n     -> No rows entered into PriceVolume b/c no PV Rows exist.");
	}
